"""
Programa OCR Reconocimiento de Caracteres con aplicacion de KNN
Creado por Julia Gabriela Chico Perez  
           Francisco Javier Reyes Beristain   
"""
...
def propiedad9(img, tImTo):                     #Creacion de la funcion
    ...
    return numuns                               #Retornamos el numero de cortes encontrados a 1/2 de la imagen de forma vertical
...
def propiedad10(img, tImTo):                    #Creacion de la funcion
    ...
    for al in range(alto):                      #For encargado de recorrer las filas de la imagen
        ...
        if compara!=val:                        #Comparamos si el valor del pixel actual es diferente a la asignacion anterior, asi sabemos si hay cortes en la imagen
            numuns=numuns+1                     #Se hace la sobrecarga del numero de cortes encontrados en la linea de busqueda
            val=(int(img[al][dista]))           #Si hay un corte en la imagen asignamos el valor a nuestra variable para seguir buscando cortes despues
    return numuns                               #Retornamos el numero de cortes encontrados a 1/4 de la imagen de forma vertical
...
def propiedad11(img, tImTo):                    #Creacion de la funcion
    ...
    for al in range(alto):                      #For encargado de recorrer las filas de la imagen
        ...
        if compara!=val:                        #Comparamos si el valor del pixel actual es diferente a la asignacion anterior, asi sabemos si hay cortes en la imagen
            numuns=numuns+1                     #Se hace la sobrecarga del numero de cortes encontrados en la linea de busqueda
            val=(int(img[al][dista]))           #Si hay un corte en la imagen asignamos el valor a nuestra variable para seguir buscando cortes despues
    return numuns                               #Retornamos el numero de cortes encontrados a 3/4 de la imagen de forma vertical
...
def propiedad12(img, tImTo):                        #Creacion de la funcion
    ...
    return numuns                                   #Retornamos el numero de cortes encontrados a 1/2 de la imagen de forma horizontal
        ...
        for anch in range(ancho):                   #For encargado de recorrer las columnas de la imagen
            if alt==altu:                           #Aquí verificamos que nuestro buscador se encuentre en la columna que necesitamos analizar
                ...
                if compara!=val:                    #Comparamos si el valor del pixel actual es diferente a la asignacion anterior, asi sabemos si hay cortes en la imagen
                    numuns=numuns+1                 #Se hace la sobrecarga del numero de cortes encontrados en la linea de busqueda
                    val=(int(img[altu][anch]))      #Si hay un corte en la imagen asignamos el valor a nuestra variable para seguir buscando cortes despues
    return numuns                                   #Retornamos el numero de cortes encontrados a 1/4 de la imagen de forma horizontal
...
def generarArchivo(datset):                         #Declaracion de la funcion
    ...
    csvsalida=open('DataPrueba.csv','w',newline='')    #Abrimos el archivo csv en el cual meteremos las propiedades
    ...
    print('Proceso finalizado')                     #Imprimimos un mensaje indicando que el proceso a concluido
...
def kNN(img, numVeci):                      #Declaración de la función
    ...
    reader = csv.reader(open('DataPrueba.csv')) #Leemos el dataSet
...
def crearDataSet():                                         #Declaracion de la funcion
    ...
    rootDir = 'informacion'                                 #Declaramos la variable rootDir la cual contiene el nombre de la carpeta en la cual se encuentran las carpetas de nuestras imagenes
    ...
    for dirName, subdirList, fileList in os.walk(rootDir):  #Este for recorrera las carpetas que contienen nuestras imagenes en la ruta especificada por "rootDir"')
        ...
        if num[12]!=' ':                                    #Validamos si el caracter no es ' '
            print('Carpetas de números : %s' % num[12])     #Aquí indicamos que carpeta exactamente estamos analizando
...
if __name__ == "__main__":                                                   #Aqui indicamos la funcion main
    ...
    while True:                                                              #Hacemos un while que se repita hasta que el usuario lo detenga
        print('Seleccione una opción')                                       #Imprimimos el menu principal
        print('1.-Buscar un numero')                                #Imprimimos el menu principal
        print('2.-Crear Data Set')                                           #Imprimimos el menu principal
        print('3.-Salir')                                                    #Imprimimos el menu principal
        opcion=int(input('¿Qué opción eliges?: '))                           #Ingresamos la opcion deseada
        if opcion==1:                                                        #Validamos si seleccionamos la opcion 1
            nomImg=input('Ingrese el nombre de la imagen a buscar: ')        #Solicitamos el nombre de la imagen a clasificar
            numVeci=int(input('Ingrese el número de vecinos: '))             #Solicitamos el numero de vecinos
            ...
            for va in range(numVeci):                                        #For para imprimir los K vecinos
                print("Número vecino: ",va+1,"\nPosición en DataSet: ", mat[va][0],"\nDistancia Euclidiana: ",mat[va][1],"\nClase: ",mat[va][2])  #Se imprimen solamente los vecinos más cercanos indicados anteriormente
